...
```bash
authentication-lambda$ sam build
```
...
```bash
authentication-lambda$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
authentication-lambda$ sam local start-api
authentication-lambda$ curl http://localhost:3000/
```
...
```bash
authentication-lambda$ sam logs -n HelloWorldFunction --stack-name authentication-lambda --tail
```
...
```bash
authentication-lambda$ cd HelloWorldFunction
...
```bash
sam delete --stack-name authentication-lambda
```
